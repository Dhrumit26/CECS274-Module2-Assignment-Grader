...
def primes(a: int, b: int):
    ...
    stop = int(b ** 0.5) + 1  # math.sqrt(b) + 1 is also an acceptable option
    P = set(range(a, b + 1))
...
def bezout_coeffs(a, b):
    ...
    :returns: dict type; a dictionary with parameters a and b as keys, and their
              corresponding Bezout coefficients as values.
...
def solve_mod_equiv(a, b, m, low, high):
    ...
    """
    if high < low:
        raise ValueError(f"solve_mod_equiv() does not support the upper bound {high} less than the lower bound {low}")
    if m < 0:
        raise ValueError(f"solve_mod_equiv() does not support negative modulo m = {m}")
    a_inv = mod_inv(a, m)
    k_low = math.ceil((low - a_inv * b)/m)
    k_high = math.floor((high - a_inv * b)/m)
