    ...
    def TestCase():
      ...
      try:
        ...
        for i in range(random.randint(10, 17)):
          ...
          boolean = arr_bool.all() and (answer_queue.size() == student_queue.size()) and str(answer_queue) == str(student_queue)
          if not boolean:
            msg += "\n\nTest FAILED."
...
f = open("/outputs/144791.json", "w")
json.dump({"id": "144791", "passed": output.passed, "log": output.logs}, f)
f.close()
