...
DEFAULT_SUBMISSIONS = "downloads/CECS 274 SEC 01 3208 (Fall 2025)/Module 2_ Array-Based Lists"
DEFAULT_TESTS_DIR = "tests/ma2"
DEFAULT_SUPPORT_DIR = "tests/ma2"
DEFAULT_OUTPUT_DIR = "tests/ma2"
DEFAULT_TESTS_DIR = "tests/ma2"
OUTPUT_DIR = "tests/ma2"
DEFAULT_RESULTS_CSV = "ma2_code_results.csv"
DEFAULT_SUMMARY_CSV = "ma2_code_summary.csv"
DEFAULT_LOG_FILE = "ma2_grading_log.txt"
DEFAULT_TIMEOUT = 500  # seconds per test (hard stop)
...
def find_tests(tests_dir: pathlib.Path) -> List[pathlib.Path]:
    # Support both Python and Shell tests
    tests: List[pathlib.Path] = []
    for p in tests_dir.iterdir():
        if not p.is_file():
            continue
        if p.name.startswith("test_") and p.suffix in (".py", ".sh"):
            tests.append(p)
    return sorted(tests)
...
def run_shell_test(
    ...
    """
    Execute a shell test. We set OUTPUTS_DIR to a tmp folder and expect the
    test to write ${OUTPUTS_DIR}/<id>.json with {"id": "...", "passed": bool, "log": "..."}.
    If not written, we treat as failure and attach stdout/stderr.
    """
    outputs_dir = work_dir / "__outputs__"
    ...
    env = os.environ.copy()
    env["OUTPUTS_DIR"] = str(outputs_dir.resolve())
    ...
    try:
        ...
        # Look for first JSON file
        candidates = sorted(outputs_dir.glob("*.json"))
        if not candidates:
            msg = "Shell test wrote no JSON to OUTPUTS_DIR.\nMake sure the script writes to ${OUTPUTS_DIR}/<id>.json."
            if out:
                msg += f"\n\nSTDOUT:\n{out[:1000]}"
            if err:
                msg += f"\n\nSTDERR:\n{err[:1000]}"
        ...
        try:
            ...
            # Append captured STDOUT/STDERR for more context
            if out:
                log = f"{log}\n\nSTDOUT:\n{out[:1000]}".strip()
            if err:
                log = f"{log}\n\nSTDERR:\n{err[:1000]}".strip()
            ...
            if err:
                msg += f"\n\nSTDERR:\n{err[:1000]}"
            return {"ok": False, "message": msg, "error": str(e)}
    ...
    with tempfile.TemporaryDirectory() as tmpd:
        ...
        with open(results_csv, "w", newline="", encoding="utf-8") as rf:
            ...
            for student_dir in iter_student_dirs(submissions_dir):
                ...
                # Run each test (py or sh) in its own subprocess with a deterministic seed
...
def run_unittest_mode(test_file: Path):
    ...
    suite.run(result)
    # If a unittest file defines zero tests, treat as failure (no false passes)
...
def run_gradescope_compat(test_file: Path, outputs_dir: Path):
    ...
    # Monkeypatch open() so writes to '/outputs/...' land in outputs_dir
    outputs_dir.mkdir(parents=True, exist_ok=True)
    ...
    # Capture prints produced by the test file
    buf_out, buf_err = io.StringIO(), io.StringIO()
    builtins.open = _patched_open  # type: ignore
    try:
        with contextlib.redirect_stdout(buf_out), contextlib.redirect_stderr(buf_err):
            # Execute the test file as a script (like Gradescope does)
            runpy.run_path(str(test_file), run_name="__main__")
    finally:
        builtins.open = _real_open  # restore

    # Find any JSON the test wrote
    candidates = sorted(outputs_dir.glob("*.json"))
    if not candidates:
        # No JSON written -> signal caller to FALL BACK to unittest
    ...
    try:
        ...
        passed = bool(data.get("passed", False))
        log = str(data.get("log", ""))
        # Append any captured stdout/stderr (helpful when tests print diagnostics)
        out_s = buf_out.getvalue()
        err_s = buf_err.getvalue()
        if out_s:
            log = f"{log}\n\nSTDOUT:\n{out_s[:1000]}".strip()
...
def main():
    ...
    # PRELOAD student's modules so later "import X" resolves to these
    preload_student_modules(student_src_dir, required_modules_csv)

    # Strategy: ALWAYS try Gradescope-style first; if no JSON, fall back to unittest.
    ...
    try:
        ...
        if gs_result.get("ok", None) is None and gs_result.get("message") == "no-json":
            ...
            # If unittest also finds no tests, include captured stdout/stderr from script run for debugging
